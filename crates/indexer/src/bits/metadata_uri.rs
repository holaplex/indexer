...
fn process_files(db: &PooledConnection, addr: &str, properties: Option<&Property>) -> Result<()> {
    ...
    if let Some(attributes) = attributes {
        ...
        } in attributes
        {
            let row = DbAttribute {
                ...
                trait_type: trait_type.as_ref().map(Into::into),
            };
...
async fn process_async<'a>(
    ...
    process_metadata_jsons(&db, &addr, fingerprint, json, &raw_content);

    process_files(&db, &addr, json.properties.as_ref());

    process_attributes(&db, &addr, json.attributes.as_ref());

    process_collection(&db, &addr, json.collection.as_ref());
    trace!("{}: {:#?}", meta_key, json);
    Ok(())
}
pub fn process(
