...
use mpl_metaplex::{
    state::{
        Key, Store, StoreConfig, WhitelistedCreator, CONFIG, MAX_STORE_CONFIG_V1_SIZE,
        MAX_STORE_SIZE, MAX_WHITELISTED_CREATOR_SIZE, PREFIX,
    },
...
async fn process_settings_uri(db: &Connection, uri: String, pda: String) -> Result<()> {
    ...
    let row = SettingsUriJson {
        store_config_pda: Borrowed(&pda),
        name: Borrowed(&json.meta.name),
        description: Borrowed(&json.meta.description),
        logo_url: Borrowed(&json.logo_url),
        banner_url: Borrowed(&json.banner_url),
        subdomain: Borrowed(&json.subdomain),
...
async fn process_store_config(client: &Client, key: Pubkey, data: Vec<u8>) -> Result<()> {
    ...
    if config.settings_uri.is_some() {
        process_settings_uri(&db, config.settings_uri.clone().unwrap(), addr.clone()).await;
    }
...
pub async fn process(client: &Client, key: Pubkey, data: Vec<u8>) -> Result<()> {
    ...
    info!("{:?}", first_byte);
    match first_byte {
        STORE => process_store(client, key, data),
        STORE_CONFIG => process_store_config(client, key, data).await,
        WHITELISTED_CREATOR => process_whitelisted_creator(client, key, data),
        _ => Ok(()),
    }
}
