...
use metaplex_auction::processor::{
    AuctionData, AuctionDataExtended, BidState, BidderMetadata, PriceFloor, BASE_AUCTION_DATA_SIZE,
};
...
pub(super) async fn process(
    ...
    let maybe_auction_data_extended = AuctionDataExtended::from_account_info(&account_info);
    let maybe_auction_data = if (account_info.data_len() >= BASE_AUCTION_DATA_SIZE) {
        AuctionData::from_account_info(&account_info).map_err(Into::into)
    } else {
        Err(anyhow!("data length shorter than BASE_AUCTION_DATA_SIZE"))
    };

    match (maybe_auction_data, maybe_auction_data_extended) {
        (Ok(_), Ok(_)) => bail!("double match!"),
        (Ok(auction_data), Err(_)) => bail!("auction data match, {:?}", auction_data),
        (Err(_), Ok(auction_data_extended)) => {
            bail!("auction data extended match {:?}", auction_data_extended)
        },
        (Err(_), Err(_)) => bail!("no match"),
    }
