...
pub(crate) async fn process(
    ...
    let offer_address = accts[1];
    let trade_state = accts[12];
    ...
    client
        ...
        .run(move |db| {
            db.build_transaction().read_write().run(|| {
                update(
                    rewards_offers::table.filter(rewards_offers::address.eq(offer_address)),
                )
                .set((
                ...
                update(
                  offers::table.filter(offers::trade_state.eq(trade_state)),
              )
              .set((
                  offers::canceled_at.eq(closed_at),
                  offers::slot.eq(slot),
              ))
              .execute(db);
