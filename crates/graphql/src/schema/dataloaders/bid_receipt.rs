use objects::{bid_receipt::BidReceipt, nft::BaseNft};
...
#[async_trait]
impl TryBatchFn<PublicKey<BaseNft>, Vec<BidReceipt>> for Batcher {
    async fn load(
        &mut self,
        addresses: &[PublicKey<BaseNft>],
    ) -> TryBatchMap<PublicKey<BaseNft>, Vec<BidReceipt>> {
        let conn = self.db()?;
