-- update _30d_floor_price for mcc
...
UPDATE
    COLLECTION_TRENDS
SET
    _30D_FLOOR_PRICE = f.floor_price
FROM
    floor_price_table f
...
--update  prev_30d_floor_price for mcc
WITH floor_price_table AS (
    SELECT
        metadata_collection_keys.collection_address AS collection,
        min(listings.price) AS floor_price
    FROM
        ...
        INNER JOIN metadata_collection_keys ON (listings.metadata = metadata_collection_keys.metadata_address)
            ...
            AND listings.marketplace_program = 'M2mx93ekt1fmXSVkTrUL9xVFHkmME8HTUi5Cyc5aF7K'
    WHERE
        listings.CREATED_AT >= (NOW() - INTERVAL '2 months')
        AND listings.CREATED_AT <= (NOW() - INTERVAL '1 months')
    GROUP BY
        metadata_collection_keys.collection_address)
UPDATE
    COLLECTION_TRENDS
SET
    PREV_30D_FLOOR_PRICE = f.floor_price
FROM
    floor_price_table f
    INNER JOIN COLLECTION_TRENDS CV ON f.COLLECTION = CV.COLLECTION
WHERE
...
-- update _30d_floor_price for non mcc
WITH floor_price_table AS (
    SELECT
...
-- update _prev_30d_floor_price for non mcc
WITH floor_price_table AS (
    SELECT
        me_metadata_collections.collection_id::text AS collection,
        min(listings.price) AS floor_price
...
-- update _30d_sales_count for mcc
WITH sales_count AS (
    SELECT
        metadata_collection_keys.collection_address AS collection,
    ...
    WHERE
        ...
        AND purchases.CREATED_AT >= (NOW() - INTERVAL '1 months')
    GROUP BY
        metadata_collection_keys.collection_address)
UPDATE
    COLLECTION_TRENDS
SET
    _30D_SALES_COUNT = s.sales_count
FROM
    sales_count s
...
-- update prev_30d_sales_count for mcc
WITH sales_count AS (
    SELECT
        metadata_collection_keys.collection_address AS collection,
        count(purchases.id) AS sales_count
    ...
    WHERE
        metadata_collection_keys.verified = TRUE
        AND purchases.marketplace_program = 'M2mx93ekt1fmXSVkTrUL9xVFHkmME8HTUi5Cyc5aF7K'
        AND purchases.CREATED_AT >= (NOW() - INTERVAL '2 months')
        AND purchases.CREATED_AT <= (NOW() - INTERVAL '1 months')
    GROUP BY
        metadata_collection_keys.collection_address)
UPDATE
    COLLECTION_TRENDS
SET
    PREV_30D_SALES_COUNT = s.sales_count
FROM
    sales_count s
    INNER JOIN COLLECTION_TRENDS CV ON s.COLLECTION = CV.COLLECTION
WHERE
...
-- update _30d_sales_count for non_mcc
WITH sales_count AS (
    SELECT
        me_metadata_collections.collection_id::text AS collection,
        count(purchases.id) AS sales_count
...
-- update prev_30d_sales_count for non_mcc
WITH sales_count AS (
    SELECT
        me_metadata_collections.collection_id::text AS collection,
        count(purchases.id) AS sales_count
