ALTER TABLE COLLECTIONS_VOLUME
    ...
    ADD COLUMN FLOOR_PRICE NUMERIC DEFAULT 0,
...
ALTER TABLE COLLECTION_TRENDS
    ...
    (coalesce(100*( NULLIF(FLOOR_PRICE,0) - NULLIF(PREV_1D_FLOOR_PRICE,0) ) / NULLIF(PREV_1D_FLOOR_PRICE,0), 0) ) 
	STORED,
    ADD COLUMN _7d_floor_price_change BIGINT GENERATED ALWAYS AS 
    (coalesce(100*( NULLIF(FLOOR_PRICE,0) - NULLIF(_PREV_7D_VOLUME,0) ) / NULLIF(_PREV_7D_VOLUME,0), 0)) 
	STORED,
    ADD COLUMN _30d_floor_price_change BIGINT GENERATED ALWAYS AS 
    (coalesce(100*( NULLIF(FLOOR_PRICE,0) - NULLIF(PREV_30D_FLOOR_PRICE,0) ) / NULLIF(PREV_30D_FLOOR_PRICE, 0), 0)) 
	STORED,
